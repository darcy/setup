...
alias gitm="nvim -c 'Gstatus | wincmd j | hide' ."
...
if begin; not string length -q -- (pgrep ssh-agent); end
   eval (ssh-agent -c)
...
#if begin; string match -v -q -- "*ssh_auth_sock" $SSH_AUTH_SOCK; end
if begin; not string length -q -- $TMUX; end
